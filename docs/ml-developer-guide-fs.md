...
* Feature computation modules under `automated_retraining/feature_engineering` folder. 
...
* Python unit tests for feature computation modules in `automated_retraining/tests/feature_engineering` folder.
* Feature engineering notebook, `automated_retraining/feature_engineering/notebooks/GenerateAndWriteFeatures.py`, that reads input dataframes, dynamically loads feature computation modules, executes their `compute_features_fn` method and writes the outputs to a Feature Store table (creating it if missing).
...
* An automated integration test is provided (in `.github/workflows/automated-retraining-run-tests-fs.yml`) that executes a multi task run on Databricks involving the feature engineering and model training notebooks.
...
1. Implement your feature module, address TODOs in `automated_retraining/feature_engineering/features` and create unit test in `automated_retraining/tests/feature_engineering`
2. Update `automated_retraining/assets/feature-engineering-workflow-asset.yml`. Fill in notebook parameters for `write_feature_table_job`.
3. Update training data path in `automated_retraining/assets/model-workflow-asset.yml`.

We expect most of the development to take place in the `automated_retraining/feature_engineering` folder.

## Iterating on ML code

### Deploy ML code and assets to dev workspace using Bundles

Refer to [Local development and dev workspace](../automated_retraining/assets/README.md#local-development-and-dev-workspace)
...
* Run `databricks configure --profile automated-retraining-dev --token --host <your-dev-workspace-url>`, passing the URL of your dev workspace.
...
* From within the root directory of the current project, use the [dbx sync](https://dbx.readthedocs.io/en/latest/guides/python/devloop/mixed/#using-dbx-sync-repo-for-local-to-repo-synchronization) tool to copy code files from your local machine into the Repo by running
  `dbx sync repo --profile automated-retraining-dev --source . --dest-repo your-repo-name`, where `your-repo-name` should be the last segment of the full repo name (`/Repos/username/your-repo-name`)

#### Running code on Databricks
You can iterate on ML code by running the provided `automated_retraining/feature_engineering/notebooks/GenerateAndWriteFeatures.py` notebook on Databricks using
...
#### Prerequisites
* Python 3.8+
* Install feature engineering code and test dependencies via `pip install -I -r automated_retraining/requirements.txt -r test-requirements.txt` from project root directory.
